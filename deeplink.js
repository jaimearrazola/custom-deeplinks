...
var IonicDeeplink = {
  ...
  route: function (paths, success, error) {
    ...
    this.onDeepLink(function (data) {
      ...
      for (var targetPath in paths) {
        ...
        //this is the first change needed since you cannot use pathData as its not a string
        //the next change I have placed inside routeMatch() but it could have been done
        //here too
  ...
  routeMatch: function (route, path) {
    ...
    //I have commented out the next line and replaced with the hardcoded parts of the route that I expect to use params for (in this case matchId). Since my routing is expecting a call of with schema appname://matches/matchId (my particular example being tennisapp://matches/12345) the array I hardcoded below tells deeplinks that the first part of the URL (the matches/ part) is not a parameter, hence the "" element, but the second part (the 12345) is where I expect to find the parameter called matchId. This of course will change according to your own apps internal routing and your deeplink needs and can be a bit hard to get right, as with any hard coding like this. It also will probably fall over entirely if you need to support more than 1 route path, fortunately I only ever deeplink to that url schema.
      
    //var routeParts = route.split('/');    //old original line
    var routeParts = ["",":matchId"];       //hard coded line
